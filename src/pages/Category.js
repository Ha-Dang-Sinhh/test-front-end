...
function Category(props) {
    ...
    const postNews = (e) => {
        e.preventDefault();
        const collectionRef = collection(db, 'newsPost');
            const collectionQuery = query(collectionRef, where('title','==', title));
            unsub = onSnapshot(collectionQuery, (snapShot) => {
                const localNewsPost = [];
                snapShot.forEach(doc => {
                    localNewsPost.push({
    ...
    return (
        <div>
            <div className="grid grid-cols-2">
                <form className="editor mx-auto w-full flex flex-col text-gray-800 border border-gray-300 p-4 shadow-lg max-w-2xl" onSubmit={postNews}>
                    <h6>Tìm kiếm theo title</h6>
                    ...
                    <div className="grid grid-cols-3 gap-4">
                        {
                            listCategory?.map((listCategoryItem,listCategoryIndex)=>{
                                return(
                                    <button className="p-6 rounded bg-yellow-500" key={listCategoryIndex} onClick={()=>{selectCategory(listCategoryItem.category)}}>{listCategoryItem.category}</button>
                                )
                            })
                        }
